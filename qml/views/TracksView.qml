...
Item {
    ...
    Timer {
        ...
        interval: 1500
        repeat: false
        onTriggered: serverManager.fetch_tracks(currentId)
    }
        ...
        function onTracks_jsonChanged() {
            ...
            try {
                ...
                for (var i = 0; i < arr.length; i++) {
                    tracksModel.append(arr[i])
                }
            } catch (e) {
                console.log("Failed to parse tracks json", e)
            }
        }
    }
    ...
    Timer {
        ...
        repeat: true
        onTriggered: {
            var status = serverManager.poll_track_upload_status()
            ...
            if (status.startsWith("uploading")) {
                ...
                progressText.text = "Track uploaded successfully!"
            ...
            var logs = serverManager.poll_track_upload_log()
    ...
    Popup {
        ...
        property string layoutsToDelete: ""
        ...
        ColumnLayout {
            ...
            Text {
                text: "Zostanie usunięty cały folder: <b>" + confirmDeletePopup.folderToDelete + "</b><br>" +
                      "Zawarte w nim layouty: <b>" + confirmDeletePopup.layoutsToDelete + "</b>"
                color: "#cbd5e1"
                font.pixelSize: 14
                Layout.fillWidth: true
                wrapMode: Text.Wrap
            ...
            RowLayout {
                ...
                Button {
                    text: "Usuń"
                    onClicked: {
                        serverManager.delete_track(currentId, confirmDeletePopup.folderToDelete)
                        delayedRefreshTimer.start()
    ...
    FileDialog {
        ...
        onAccepted: {
            ...
            serverManager.upload_track(currentId, uploadDialog.selectedFile.toString())
        ...
        GridView {
            ...
            delegate: Card {
                ...
                ColumnLayout {
                    ...
                    Text {
                        text: "Folder: " + model.track_folder + (model.layout ? " | Layout: " + model.layout : "")
                        color: Theme.textSecondary
                        font.pixelSize: 12
                        font.family: Theme.fontFamily
                        elide: Text.ElideRight
                    ...
                    RowLayout {
                        ...
                        DangerButton {
                            ...
                            onClicked: {
                                ...
                                if (layouts.length > 0) {
                                    confirmDeletePopup.layoutsToDelete = layouts.join(", ");
                                } else {
                                    confirmDeletePopup.layoutsToDelete = "brak (tylko domyślny)";
                                }
